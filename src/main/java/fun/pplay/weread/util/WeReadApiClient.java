...
public class WeReadApiClient {
    ...
    private volatile boolean cookieInitialized = false;
    private final ObjectMapper objectMapper;

    @Autowired
    public WeReadApiClient(ReactiveSettingFetcher settingFetcher,ObjectMapper objectMapper) {
        this.httpClient = HttpClients.createDefault();
        this.objectMapper = objectMapper;
        this.settingFetcher = settingFetcher;
        this.cookie = null;  // 延迟初始化
    }
    // 新增的测试用构造函数
    public WeReadApiClient(String cookie) {
        this.httpClient = HttpClients.createDefault();
        this.settingFetcher = null; // 测试时不使用
        this.objectMapper = new ObjectMapper();
        this.cookie = cookie;
    }
    private String fetchCookie() {
        try {
            return settingFetcher.get("weread") // 修改配置组名称为正确的值
                .map(setting -> {
                    log.info("11111");
                    // 1. 获取整个配置数据
                    JsonNode configData = setting;
                    log.info("22222");
                    // 2. 提取 weread 组配置
                    JsonNode wereadGroup = configData.get("weread");
                    if (wereadGroup == null || wereadGroup.isNull()) {
                        throw new RuntimeException("配置中缺少 weread 组");
                    }

                    // 3. 提取 keywords 字段值
                    JsonNode keywordsNode = wereadGroup.get("keywords");
                    if (keywordsNode == null || keywordsNode.isNull()) {
                        throw new RuntimeException("weread 组中缺少 keywords 字段");
                    }

                    // 4. 转换为字符串并验证
                    String cookie = keywordsNode.asText();
                    if (cookie.isBlank()) {
                ...
                .orElseThrow(() -> new RuntimeException("获取 Cookie 超时"));
        } catch (Exception e) {
            throw new RuntimeException("获取 Cookie 失败: " + e.getMessage(), e);
        }
    }



    private void ensureCookieInitialized() {
            ...
            synchronized (this) {
                if (!cookieInitialized) {
                    try {
                        ...
                        throw new RuntimeException("初始化 Cookie 失败: " + e.getMessage(), e);
                    }
                }
            }
        }
    ...
    private JsonNode executeGetRequest(String url) throws Exception {
        ensureCookieInitialized(); // 加入 Cookie 初始化检查
